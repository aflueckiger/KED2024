...
## Programming can be absolutely captivating! :v: {data-background="var(--green)"}
...
# Programming Concepts & <br>Python Syntax :brain:
...
::: columns
::: column
-   Windows user only: connect to WSL Ubuntu left-lower corner

-   load Python environment `.venv`

-   control bar for global actions on top

-   run code with ▶️ left to the cell or with `CTRL+Enter`

-   output shows below each cell

-   additional information at the bottom (e.g., variables)
:::

::: column
![The output "Hello World!" should look like in the screenshot.](../images/vs_code_hello_world_full.jpg)
:::
:::
...
3.  Windows User only: Make sure that you are connected to `WSL: Ubuntu` (blue badge lower-left corner). If not, click on the badge and select `New WSL Window`.
4.  Open the `KED2024` folder via `File > Open Folder`.
5.  Make sure that you have loaded the correct Python environment `.venv`.

## In-class: Run your first Python Program {data-background="#3c70b5"}

1.  Create a new file `Jupyter Notebook`\` with the following content, save it as `hello_world.ipynb` in the `KED2024` folder. Then, run the code.
...
2.  Does the output looks like the screenshot on the previous slide? If the execution doesn't work as expected, ask me or your neighbour. There might be a technical issue.
